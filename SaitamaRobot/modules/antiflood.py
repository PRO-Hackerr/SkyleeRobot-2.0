...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        msg.reply_text(
            "I can't restrict people here, give me permissions first! Until then, I'll disable Flood Control."
        )
        sql.set_flood(chat.id, 0)
        return "<b>{}:</b>" \
               "\n#INFO" \
               "\nDon't have enough permission to restrict users so automatically disabled Flood Control".format(chat.title)
...
def set_flood(update, context) -> str:
    ...
    if len(args) >= 1:
        ...
        if val == "off" or val == "no" or val == "0":
            ...
            if conn:
                text = message.reply_text(
                    "Flood Control has been disabled in {}.".format(chat_name))
            else:
                text = message.reply_text("Flood Control has been disabled.")
            ...
            if amount <= 0:
                ...
                return "<b>{}:</b>" \
                       "\n#SETFLOOD" \
                       "\n<b>Admin:</b> {}" \
                       "\nDisable Flood Control.".format(html.escape(chat_name), mention_html(user.id, html.escape(user.first_name)))

            elif amount <= 3:
                send_message(
                    update.effective_message,
                    "Flood Control must be either 0 (disabled) or number greater than 3!"
                )
                return ""
                ...
                if conn:
                    text = message.reply_text(
                        "Flood Control has been set to {} in chat: {}".format(
                            amount, chat_name))
                else:
                    text = message.reply_text(
                        "Successfully updated Flood Control limit to {}!".format(
                            amount))
                return "<b>{}:</b>" \
                       "\n#SETFLOOD" \
                       "\n<b>Admin:</b> {}" \
                       "\nSet Flood Control to <code>{}</code>.".format(html.escape(chat_name),
        ...
        message.reply_text((
            "Use `/setflood number` to enable Flood Control.\nOr use `/setflood off` to disable Flood Control!."
        ),
                           parse_mode="markdown")
    return ""
...
def set_flood_mode(update, context):
    ...
    if args:
        if args[0].lower() == 'ban':
            ...
            if len(args) == 1:
                teks = """It looks like you tried to set time value for Flood Control but you didn't specified time; Try, `/setfloodmode tban <timevalue>`.
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
            ...
            if len(args) == 1:
                teks = update.effective_message, """It looks like you tried to set time value for Flood Control but you didn't specified time; Try, `/setfloodmode tmute <timevalue>`.
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
        ...
        return "<b>{}:</b>\n" \
                "<b>Admin:</b> {}\n" \
                "Has changed Flood Control mode. User will {}.".format(settypeflood, html.escape(chat.title),
                                                                            mention_html(user.id, html.escape(user.first_name)))
    else:
        getmode, getvalue = sql.get_flood_setting(chat.id)
        if getmode == 1:
...
def __chat_settings__(chat_id, user_id):
    ...
    if limit == 0:
        ...
        return "Flood Control has been set to`{}`.".format(limit)


__help__ = """
Flood Control allows you to take action on users that send more than x messages in a row. Exceeding the set flood \
...
__handlers__ = [(FLOOD_BAN_HANDLER, FLOOD_GROUP), SET_FLOOD_HANDLER,
                FLOOD_HANDLER, SET_FLOOD_MODE_HANDLER] anti
