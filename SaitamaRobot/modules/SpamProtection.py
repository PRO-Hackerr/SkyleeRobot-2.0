...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DRAGONS:
        message.reply_text(
            "I spy, with my little eye... seen a disaster!"
        )
        return
    ...
    if user_id in [777000, 1087968824]:
        message.reply_text("Haha! Gbanning Telegram Native Tech, Fumnny")
        return
    ...
    try:
        bot.send_message(
            ...
            f"</b>Appeal Chat:</b> @SkyleeHelpChat",
            parse_mode=ParseMode.HTML)
    except:
...
def __user_info__(user_id):
    ...
    if is_gbanned:
        ...
        if user.reason:
            text += f"\n<b>Reason:</b> <code>{html.escape(user.reason)}</code>"
        text += f"\n<b>Appeal Chat:</b> @SkyleeHelpChat"
    else:
        text = text.format("???")
    return text
...
__help__ = f"""
*Admins only:*
 • `/spamprotection <on/off/yes/no>`*:* Will toggle our spamprotection tech or return your current settings.

SpamProtection, used by bot devs to ban spammers across all groups. This helps protect \
you and your groups by removing spam flooders as quickly as possible.
*Note:* Users can appeal gbans or report spammers at @SkyleeHelpChat
