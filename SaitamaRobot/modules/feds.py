...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        #if fednam == 'TeamSkyleeDevs':
        #fed_id = "TeamSkyleeDevs"

        x = sql.new_fed(user.id, fed_name, fed_id)
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @SkyleeHelpChat if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @SkyleeHelpChat should this problem persist!"
            )
            return
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in DRAGONS:
        ...
        update.effective_message.reply_text(
            "Only federation owners can do this!, Create one first!")
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @SkyleeHelpChat."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @SkyleeHelpChat !"
            )
            return
...
def fed_owner_help(update: Update, context: CallbackContext):
 ...
 • `/fedsubs`*:* Shows the feds your group is subscribed to `(Not Completed)`""",
        parse_mode=ParseMode.MARKDOWN)
